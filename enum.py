...
def conductHeavyNmap(target):
    ...
    TCPSCAN = "nmap -T4 -Pn -A -sV --top-ports 10000 %s" % target
    try:
        tcpResults = subprocess.check_output(TCPSCAN, shell=True)
...
def pop3(target, ports):
    ...
    for port in ports:
        port = port.split("/")[0]
        portString += "%s," % port
    NMAPSCAN = "nmap -T4 -p %s -sV --script=\"pop3-capabilities,pop3-ntlm-info\" %s" % (portString, target)
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
...
def imap(target, ports):
    ...
    for port in ports:
        port = port.split("/")[0]
        portString += "%s," % port
    NMAPSCAN = "nmap -T4 -p %s -sV --script=\"imap-capabilities,imap-ntlm-info\" %s" % (portString, target)
    try:
        nmapscanResults = subprocess.check_output(NMAPSCAN, shell=True)
    ...
    BRUTE = "nmap -T4 -p %s --script=\"imap-brute\" %s" % (portString, target)
    try:
        bruteResults = subprocess.check_output(BRUTE, shell=True)
...
def smb(target, ports):
    ...
    SCAN1 = "nmap -T4 -sV -sC --script=\"smb-vuln-*,samba-vuln-*\" -p 445,139 %s" % target
    SCAN2 = "nmap -T4 -sU -sV -sC --script=\"smb-vuln-*\" -p U:137,T:139 %s" % target
    try:
        scan1Results = subprocess.check_output(SCAN1, shell=True)
